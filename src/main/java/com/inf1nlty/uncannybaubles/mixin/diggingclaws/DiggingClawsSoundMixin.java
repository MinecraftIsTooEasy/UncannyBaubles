...
public abstract class DiggingClawsSoundMixin {
    ...
    {
        if (this.ub$shouldUseDiggingClawsSound(x, y, z)) {
    ...
    @Unique
    private boolean ub$shouldUseDiggingClawsSound(float x, float y, float z) {
        if (this.mc.thePlayer.getHeldItem() != null || !DiggingClawsUtil.hasEquipped(this.mc.thePlayer)) return false;
        ...
        return DiggingClawsUtil.isValidTarget(block, metadata);
    }
}
